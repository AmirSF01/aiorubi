...
if TYPE_CHECKING:
    ...
    from aiorubi.types import InputFile
...
class AiohttpSession(BaseSession):
    ...
    async def create_session(self) -> ClientSession:
        ...
        if self._session is None or self._session.closed:
            self._session = ClientSession(
                ...
                headers={
                    USER_AGENT: f"{SERVER_SOFTWARE} aiorubi/{__version__}",
                },
            )
    ...
    def build_form_data(self, bot: Bot, method: RubikaMethod[RubikaType]) -> dict[str, Any]:
        return {
            ...
            if (prepared_value := self.prepare_value(value, bot=bot)) is not None
        }
    ...
    async def make_request(
        ...
        try:
            ...
            raise RubikaNetworkError(method=method, message="Request timeout error") from e
        except ClientError as e:
            raise RubikaNetworkError(method=method, message=f"{type(e).__name__}: {e}") from e
