...
from aiorubi.client.session.middlewares.base import (
...
class RequestMiddlewareManager(Sequence[RequestMiddlewareType]):
    ...
    def wrap_middlewares(
        self,
        callback: NextRequestMiddlewareType[RubikaType],
        **kwargs: Any,
    ) -> NextRequestMiddlewareType[RubikaType]:
        middleware = partial(callback, **kwargs)
        for m in reversed(self._middlewares):
            middleware = partial(m, middleware)
        return cast(NextRequestMiddlewareType[RubikaType], middleware)