...
from aiorubi.exceptions import RubikaAPIError
from aiorubi.fsm.middleware import FSMContextMiddleware
from aiorubi.fsm.storage.base import BaseEventIsolation, BaseStorage
from aiorubi.fsm.storage.memory import DisabledEventIsolation, MemoryStorage
from aiorubi.fsm.strategy import FSMStrategy
from aiorubi.methods import GetUpdates, RubikaMethod
from aiorubi.types import Update, BotInfo
from aiorubi.types.update import UpdateTypeLookupError
from aiorubi.utils.backoff import Backoff, BackoffConfig
...
class Dispatcher(Router):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Rubika API provides originally only one event type - Update
        # For making easily interactions with events here is registered handler which helps
        # to separate Update to different event types like Message, EditedMessage, RemovedMessage etc.
        self.update = self.observers["update"] = RubikaEventObserver(
            router=self,
            event_name="update",
        )
    ...
    async def feed_update(self, bot: Bot, update: Update, **kwargs: Any) -> Any:
        ...
        try:
            ...
            loggers.event.info(
                "Update type=%s chat_id=%s message_id=%s is %s. Duration %d ms by bot id=%s",
                update.type,
                update.chat_id,
                update.message_id,
    ...
    async def _listen_updates(
        ...
        if bot.session.timeout:
            ...
            kwargs["request_timeout"] = int(bot.session.timeout)
        failed = False
        while True:
            try:
                ...
                failed = True
                # In cases when Rubika Bot API was inaccessible don't need to stop polling
            ...
            # Rubika returns next_offset_id in response to confirm received updates.
            # Pass it as offset_id in the next request to avoid receiving duplicates.
            if updates.next_offset_id:
                get_updates.offset_id = updates.next_offset_id
            if not updates:
    ...
    async def _listen_update(self, update: Update, **kwargs: Any) -> Any:
        ...
        try:
            ...
            warnings.warn(
                ...
                "Seems like Rubika Bot API was updated and you have "
                "installed not latest version of aiorubi framework"
    ...
    @classmethod
    async def silent_call_request(cls, bot: Bot, result: RubikaMethod[Any]) -> None:
        """
    ...
    async def _process_update(
        ...
        **kwargs: Any,
        ...
        :param call_answer: need to execute response as Rubika method (like answer into webhook)
        :param kwargs: contextual data for middlewares, filters and handlers
        :return: status
        """
        try:
            response = await self.feed_update(bot, update, **kwargs)
            if call_answer and isinstance(response, RubikaMethod):
                await self.silent_call_request(bot=bot, result=response)

        except Exception as e:  # noqa: BLE001
            loggers.event.exception(
                "Cause exception while process update type=%s chat_id=%s message_id=%s by bot id=%s\n%s: %s",
                update.type,
                update.chat_id,
                update.message_id,
                bot.id,
    ...
    async def _polling(
        ...
        **kwargs: Any,
        ...
        :param bot:
        :param limit: Maximum number of updates to fetch per request
        :param polling_interval: Delay between polling requests when no updates are available
        ...
        :return:
        """
        user: BotInfo = await bot.me()
        loggers.dispatcher.info(
            "Run polling for bot @%s id=%s - %r",
            user.username,
            bot.id,
            user.bot_title,
        )
        ...
        try:
            ...
            loggers.dispatcher.info(
                "Polling stopped for bot @%s id=%s - %r",
                user.username,
                bot.id,
                user.bot_title,
            )
        ...
        **kwargs: Any,
        ...
        def process_response(task: Future[Any]) -> None:
            warnings.warn(
                ...
                "Rubika is waiting for response only first 60 seconds and then re-send update.\n"
            ...
            result = task.result()
            if isinstance(result, RubikaMethod):
        ...
        try:
            ...
            if process_updates.done():
                ...
                if isinstance(response, RubikaMethod):
                    return response
