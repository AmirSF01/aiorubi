...
from aiorubi.dispatcher.middlewares.manager import MiddlewareManager
from aiorubi.exceptions import UnsupportedKeywordArgument
...
class RubikaEventObserver:
    """
    Event observer for Rubika events
    ...
    def _resolve_middlewares(self) -> list[MiddlewareType[RubikaObject]]:
        middlewares: list[MiddlewareType[RubikaObject]] = []
    ...
    def check_root_filters(self, event: RubikaObject, **kwargs: Any) -> Any:
        return self._handler.check(event, **kwargs)

    async def trigger(self, event: RubikaObject, **kwargs: Any) -> Any:
        """
