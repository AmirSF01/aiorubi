...
from aiorubi.dispatcher.dispatcher import Dispatcher
from aiorubi.dispatcher.event.handler import CallableObject, CallbackType
from aiorubi.dispatcher.flags import extract_flags_from_object
...
from aiorubi.fsm.storage.memory import MemoryStorageRecord
from aiorubi.types import RubikaObject, Update
from aiorubi.utils.class_attrs_resolver import (
    ClassAttrsResolver,
    get_sorted_mro_attrs_resolver,
)

if TYPE_CHECKING:
    from aiorubi.dispatcher.event.bases import NextMiddlewareType
...
class SceneConfig:
    ...
    inline_message_without_state: bool | None = None
...
class Scene:
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        ...
        inline_message_without_state = kwargs.pop("inline_message_without_state", None)
        ...
        for base in cls.__bases__:
            ...
            if inline_message_without_state is None:
                inline_message_without_state = parent_scene_config.inline_message_without_state
        ...
        cls.__scene_config__ = SceneConfig(
            ...
            inline_message_without_state=inline_message_without_state,
    ...
    def add_to_router(cls, router: Router) -> None:
        ...
        for observer_name in used_observers:
            if scene_config.inline_message_without_state and observer_name == "callback_query":
                continue
...
class SceneWizard:
    """
    A class that represents a wizard for managing scenes in a Rubika bot.
    ...
    def __init__(
        ...
        :param event: The RubikaObject represents the event.
...
class SceneRegistry:
    """
    A class that represents a registry for scenes in a Rubika bot.
    """
    ...
    async def _update_middleware(
        self,
        handler: NextMiddlewareType[RubikaObject],
        event: RubikaObject,
        data: dict[str, Any],
    ) -> Any:
...
class OnMarker:
    ...
    new_message = ObserverMarker("new_message")
    updated_message = ObserverMarker("updated_message")
    removed_message = ObserverMarker("removed_message")
    inline_message = ObserverMarker("inline_message")
