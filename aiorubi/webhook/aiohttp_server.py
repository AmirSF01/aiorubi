...
from aiohttp import JsonPayload, Payload, web
from aiohttp.typedefs import Handler
from aiohttp.web_app import Application
from aiohttp.web_middlewares import middleware

from aiorubi import Bot, Dispatcher, loggers
...
def setup_application(app: Application, dispatcher: Dispatcher, /, **kwargs: Any) -> None:
    ...
    :param dispatcher: aiorubi dispatcher
...
class BaseRequestHandler(ABC):
    def __init__(
        ...
        **data: Any,
        ...
        :param dispatcher: instance of :class:`aiorubi.dispatcher.dispatcher.Dispatcher`
        :param handle_in_background: immediately responds to the Rubika instead of
    ...
    def register(self, app: Application, /, *paths: str, **kwargs: Any) -> None:
        """
    ...
    # @abstractmethod
    # def verify_secret(self, rubika_secret_token: str, bot: Bot) -> bool:
    #     pass
    ...
    def _build_response_writer(
        self,
        bot: Bot,
        result: RubikaMethod[RubikaType] | None,
    ) -> Payload:
        # Unlike Telegram, Rubika does not support responding with API calls via webhook response.
        return JsonPayload({"status": "OK"})

    async def _handle_request(self, bot: Bot, request: web.Request) -> web.Response:
        result: RubikaMethod[Any] | None = await self.dispatcher.feed_webhook_update(
            bot,
    ...
    async def handle(self, request: web.Request) -> web.Response:
        ...
        # if not self.verify_secret(request.headers.get("X-Rubika-Bot-Api-Secret-Token", ""), bot):
...
class SimpleRequestHandler(BaseRequestHandler):
    def __init__(
        ...
        **data: Any,
        ...
        :param bot: instance of :class:`aiorubi.client.bot.Bot`
        """
    ...
    #         return secrets.compare_digest(rubika_secret_token, self.secret_token)
    #     return True
...
class TokenBasedRequestHandler(BaseRequestHandler):
    ...
    def register(self, app: Application, /, *paths: str, **kwargs: Any) -> None:
        ...
        :param kwargs:
        """
        if not all("{bot_token}" in path for path in paths):
            raise ValueError("All paths should contains '{bot_token}' substring")
        super().register(app, *paths, **kwargs)
